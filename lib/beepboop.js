...
BeepBoop.prototype._onMessage = function (message) {
    if (this._isChatMessage(message) &&
		...
		if (StartupGuy._isInvokingStartup(message)) {
			var self = this;
			var channel = self.prototype._getChannelById(originalMessage.channel);
			var myStartup = StartupGuy._replyWithStartup(message);
			self.prototype.postMessageToChannel(channel.name, myStartup, { 'slackbot': true, username: 'Startup Guy', icon_emoji: ':startupbot:' });
