...
/*    if (this._isChatMessage(message) &&
	...
	BeepBoop.postMessage(data.channel, 'Ahoy', { 'slackbot': true, username: 'Startup Guy', icon_emoji: ':startupbot:' })
    var character = bots.doYouHaveSomethingToSay(data)
    if (character) {
      BeepBoop.postMessage(data.channel, character.quip, character.params)
    }
  }
};
